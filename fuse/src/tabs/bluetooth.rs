...
fn create_power_section() -> GtkBox {
    ...
    let power_row = create_toggle_row_with_switch(
        "Bluetooth",
        if is_powered { "Bluetooth is enabled" } else { "Bluetooth is disabled" },
        toggle_switch,
    );
...
fn create_device_row(device: &BluetoothDevice, is_paired: bool) -> GtkBox {
    ...
    let mac_label = Label::new(Some(&device.mac_address));
...
fn start_scan(container: &GtkBox) {
    ...
    gtk4::glib::timeout_add_local(std::time::Duration::from_secs(2), move || {
        ...
        if devices.is_empty() {
            let placeholder = Label::new(Some("No devices found"));
            placeholder.add_css_class("placeholder");
            placeholder.set_xalign(0.0);
