...
use crate::window::VitalsWindow;
use crate::data::EnhancedSystemMonitor;
use crate::core::ColorConfig;
use std::rc::Rc;
use std::cell::RefCell;

pub struct VitalsApp {
    application: Application,
    window: Rc<RefCell<Option<VitalsWindow>>>,
...
impl VitalsApp {
    pub fn new(application: Application) -> Self {
        ...
        // Load CSS with colors on startup
        ...
        Self {
            ...
            _monitors: Rc::new(RefCell::new(Vec::new())),
        }
    }
    ...
    pub fn activate(&mut self) {
        ...
        // Create main window
        let window = VitalsWindow::new(&self.application);
        
        // Setup data channel
        let (sender, receiver) = async_channel::unbounded();
        
        // Start system monitor
        let monitor = EnhancedSystemMonitor::new(sender);
        monitor.start();
        
        // Setup data receiver
        let window_clone = Rc::new(window);
        let window_for_receiver = window_clone.clone();
        
        glib::spawn_future_local(async move {
            while let Ok(data) = receiver.recv().await {
                window_for_receiver.update_data(&data);
            }
        });
        
        // Start color monitoring
        let css_provider_monitor = self._css_provider.clone();
        let monitors = start_color_monitoring(css_provider_monitor);
        *self._monitors.borrow_mut() = monitors;
        
        // Show window
        window_clone.present();
        
        // Store window reference - we can't unwrap Rc here, so just keep the reference
        // The window will be kept alive by GTK's reference counting
        drop(window_clone);
    }
}
